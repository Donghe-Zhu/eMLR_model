...
for (i_variable in variables) {
  ...
  variable_data_tibble <-
    ...
    filter(!!sym(i_variable) < 10000)
  
  # convert longitudes
  variable_data_tibble <- variable_data_tibble %>%
    mutate(lon = if_else(lon < 20, lon + 360, lon))
  
  # mutate year and month
  variable_data_tibble <- variable_data_tibble %>%
  ...
  for (i_year in years) {
    ...
    # select cmorized data for that year
    ...
    model_subset <-
      inner_join(grid_subset, variable_data_tibble_year)
    ...
    # transform variables unit from mol/m3 to mmol/kg
    if (i_variable != "so" && i_variable != "thetao") {
      model_obs_interpo <- model_obs_interpo %>%
        mutate(!!sym(i_variable) := !!sym(i_variable) * dens_factor)
      variable_data_tibble_year <- variable_data_tibble_year %>%
        mutate(!!sym(i_variable) := !!sym(i_variable) * dens_factor)
    }
...
ggplot() +
  geom_path(
    data = model_obs_interpo %>%
      filter(lat == 0.5, lon == 315.5, !is.na(!!sym(i_variable))) %>%
      arrange(depth),
    aes(!!sym(i_variable), depth, col = "model")
  ) +
  geom_point(
    data = model_obs_interpo %>%
      filter(lat == 0.5, lon == 315.5),
    aes(!!sym(
