...
for (i in 1:8) {
  ...
  for (j in 1:4) {
    variable_annual <- GLODAP_model %>%
      ...
      drop_na() %>%
      mutate(
        offset = !!sym(model_var[i]) -!!sym(obs_var[i]),
        offset_log = !!sym(model_var[i]) / !!sym(obs_var[i])
      ) %>%
      group_by(lat, lon) %>%
      summarise(offset = mean(offset),
                offset_log = mean(offset_log)) %>%
      ungroup()
    
    # plot mean spatial distribution (model - obs)
    limit <- max(abs(variable_annual$offset)) * c(-1, 1)
    print(
      ggplot() +
    ...
    # plot mean spatial distribution (ratio: model / obs)
    limit <-
      c(2 - max(abs(variable_annual$offset_log)), max(abs(variable_annual$offset_log)))
    print(
      ggplot() +
        geom_raster(data = variable_annual, aes(lon, lat, fill = offset_log)) +
        scale_fill_scico(
          palette = "vik",
          name = "offset ratio",
          limit = limit,
        ...
        labs(
          title = paste(
            ...
            "vs",
            obs_var[i],
            "mean offset ratio spatial distribution"
          ),
          subtitle = paste("Depth intervals:", intervals[j], "-", intervals[j +
                                                                              1]),
          x = "Longitude",
