...
for (i_year in years) {
  ...
  for (i_variable in variables) {
    ...
    GLODAP <- natural_join(GLODAP, model_obs_interpo, by = c("year", "month", "lat", "lon", "depth"), jointype = "FULL")
    ...
    ggsave(here::here("output/model_subsetting/surface_model_GLODAP_obs",
    ...
    if (i_variable == "DIC") {
      ...
      variable_data_tibble_annual_average %>%
        write_csv(here::here(paste("data/model/",model_ID, sep = ""),
...
stats_summary %>%
  write_csv(here::here("data/model/summary_stats",
...
ggplot() +
  geom_path(
    ...
    aes(!!sym(paste(i_variable, "interpolate", sep = "_")), depth, col = "interpolated")
  ) +
  geom_point(
    data = model_obs_interpo %>%
      filter(lat == 1.5, lon == 316.5, !is.na(!!sym(i_variable)), month == 1) %>%
